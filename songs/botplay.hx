...
function postCreate() {
	for (strumLine in strumLines) {
		...
		botplayTxt.setFormat(Paths.font("SuperMario256.ttf"), 32, scripts.get("uiParams").mainColor, FlxTextAlign.CENTER, FlxTextBorderStyle.OUTLINE, scripts.get("uiParams").secondaryColor);
...
function regenHud() {
	for (strumLine in strumLines) {
		strumLine.extra.get('botplayTxt')?.setFormat(Paths.font("SuperMario256.ttf"), 32, scripts.get("uiParams").mainColor, FlxTextAlign.CENTER, FlxTextBorderStyle.OUTLINE, scripts.get("uiParams").secondaryColor);
	}
}